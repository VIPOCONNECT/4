...
let inactivityTimeout = 3000; // 3 שניות

// משתנה גלובלי לשמירת סוג האתר הנוכחי
let currentSiteType = 'business';
...
function handleMouseUp() {
  isDragging = false;
  // לא מפעילים סיבוב אוטומטי מיד, אלא מחכים לטיימר חוסר פעילות
}
...
function initSitePreview() {
  // הצגת האתר הראשון (business) כברירת מחדל
  currentSiteType = 'business';
  const siteData = sitesData[currentSiteType];
  ...
  // עדכון ה-iframe
  document.getElementById('site-preview-iframe').src = siteData.previewUrl;
}
...
const sitesData = {
  ...
  portfolio: {
    ...
    previewImage: "https://vipoconnect.github.io/2/",
    rating: "(19)",
    features: [
  ...
  blog: {
    ...
    previewImage: "https://vipoconnect.github.io/-/",
    rating: "(22)",
    features: [
      "עיצוב מקצועי ואמין",
      "מערכת תיאום פגישות",
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // הוספת מאזין אירוע לכפתור "גלה אתרים"
  const discoverButton = document.querySelector('.cta-primary');
  if (discoverButton) {
    discoverButton.addEventListener('click', function() {
      // גלילה לקוביה
      document.getElementById('cube-showcase').scrollIntoView({ behavior: 'smooth' });
    });
  }
});
...
function startAutoRotate() {
  ...
  autoRotateInterval = setInterval(() => {
    ...
    // הוספת תנועה על ציר X כדי לראות את הפאות העליונה והתחתונה
    rotationX = 20 * Math.sin(angle / 30);
    
    updateCubeRotation();
  }, 30);
}

// אתחול כפתורי ניווט בין אתרים
function initNavigationButtons() {
  const nextSiteBtn = document.getElementById('next-site');
  const prevSiteBtn = document.getElementById('prev-site');
  
  if (!nextSiteBtn || !prevSiteBtn) {
    console.log("Navigation buttons not found in DOM");
    return;
  }
  
  console.log("Navigation buttons found:", nextSiteBtn, prevSiteBtn);
  
  // מערך של סוגי האתרים לפי הסדר
  const siteTypes = Object.keys(sitesData); // ['business', 'ecommerce', 'portfolio', 'blog', 'landing', 'restaurant']
  
  // כפתור האתר הבא
  nextSiteBtn.addEventListener('click', () => {
    console.log("Next button clicked. Current site type:", currentSiteType);
    // מציאת האינדקס של האתר הנוכחי
    const currentIndex = siteTypes.indexOf(currentSiteType);
    // חישוב האינדקס של האתר הבא (במעגליות)
    const nextIndex = (currentIndex + 1) % siteTypes.length;
    // עדכון תצוגת האתר
    const nextSiteType = siteTypes[nextIndex];
    console.log("Switching to next site type:", nextSiteType, "with data:", sitesData[nextSiteType]);
    updateSitePreview(nextSiteType);
    
    // אנימציה לכפתור
    const btnElement = nextSiteBtn.querySelector('.btn-3d');
    if (btnElement) {
      btnElement.style.animation = 'none';
      setTimeout(() => {
        btnElement.style.animation = 'pulse-3d 3s infinite ease-in-out';
      }, 10);
    }
  });
  
  // כפתור האתר הקודם
  prevSiteBtn.addEventListener('click', () => {
    console.log("Previous button clicked. Current site type:", currentSiteType);
    // מציאת האינדקס של האתר הנוכחי
    const currentIndex = siteTypes.indexOf(currentSiteType);
    // חישוב האינדקס של האתר הקודם (במעגליות)
    const prevIndex = (currentIndex - 1 + siteTypes.length) % siteTypes.length;
    // עדכון תצוגת האתר
    const prevSiteType = siteTypes[prevIndex];
    console.log("Switching to previous site type:", prevSiteType, "with data:", sitesData[prevSiteType]);
    updateSitePreview(prevSiteType);
    
    // אנימציה לכפתור
    const btnElement = prevSiteBtn.querySelector('.btn-3d');
    if (btnElement) {
      btnElement.style.animation = 'none';
      setTimeout(() => {
        btnElement.style.animation = 'pulse-3d 3s infinite ease-in-out 0.5s';
      }, 10);
    }
  });
}

// אתחול כפתורי התצוגה (מחשב/טאבלט/מובייל)
...
// הוספת אתחול כפתורי הניווט לפונקציית האתחול הראשית
document.addEventListener('DOMContentLoaded', function() {
  // אתחול הקוביה התלת-ממדית
  init3DCube();
  ...
  // אתחול תצוגת האתר הראשוני
  initSitePreview();
  
  // אתחול כפתורי התצוגה (מחשב/טאבלט/מובייל)
  initPreviewControls();
  
  // אתחול כפתורי הניווט בין אתרים
  setTimeout(() => {
    initNavigationButtons();
    console.log("Navigation buttons initialized after delay");
  }, 1000);
  
  // אתחול כפתור הגלילה לקוביה
