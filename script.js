...
function updatePriceBasedOnParticipants() {
    // טבלת מחירים לפי מספר משתתפים - טבלה מאוחדת
    const priceTable = [
    ...
    console.log(`עדכון מחיר: עבור ${totalParticipants} משתתפים, המחיר הוא ${finalPrice} ש"ח`);
}
...
// פונקציה לסימולציית הצטרפות חדשה כל כמה שניות
// פונקציה להצגת הודעה על משתתף חדש
function showNewParticipantNotification(firstName, lastName) {
    // בדיקה אם יש מכל הודעות
    let notificationContainer = document.getElementById('notification-container');
    if (!notificationContainer) {
        // יצירת מכל הודעות חדש
...
// פונקציה לעדכון מונה המשתתפים והמחיר
function updateParticipantsCounter() {
    // עדכון מספר המשתתפים בכל הרכיבים בדף
    ...
    // עדכון המחיר בהתאם למספר המשתתפים לפי הטבלה המאוחדת
    syncParticipantsAndPrice();
}

function simulateNewParticipants() {
    // הגרלת מספר אקראי בין 1 ל-3 של משתתפים חדשים
    const newParticipants = Math.floor(Math.random() * 3) + 1;
    
    // הוספת המשתתפים למספר הכולל
    totalParticipants += newParticipants;
    
    // יצירת שמות אקראיים לצורך ההודעה
    const firstNames = ["יוסי", "דני", "רוני", "שרה", "יעל", "אילן", "מירי", "איריס", "ירון", "נועה"];
    const lastNames = ["כהן", "לוי", "ביטון", "מלכה", "אברהם", "דויד", "שמש", "רוזן", "גולן", "שושן"];
    
    const randomFirstName = firstNames[Math.floor(Math.random() * firstNames.length)];
    const randomLastName = lastNames[Math.floor(Math.random() * lastNames.length)];
    
    // הצגת הודעה על משתתף חדש
    showNewParticipantNotification(randomFirstName, randomLastName);
    
    // עדכון מונה המשתתפים והמחיר
    updateParticipantsCounter();
    
    // תזמון הפונקציה הבאה בטווח של 5-15 שניות
    const nextTimeout = Math.floor(Math.random() * 10000) + 5000;
    setTimeout(simulateNewParticipants, nextTimeout);
    
    console.log(`הצטרפו ${newParticipants} משתתפים חדשים. סה״כ ${totalParticipants} משתתפים. הודעה הבאה בעוד ${Math.round(nextTimeout/1000)} שניות`);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // התחלת סימולציית משתתפים חדשים לאחר 3 שניות
    setTimeout(function() {
        simulateNewParticipants();
    }, 3000);
...
// פונקציה ליצירת רשימת משתתפים ראשונית
function generateInitialParticipantsList() {
    // רשימת שמות פרטיים ומשפחות אקראיים
    const firstNames = ["יוסי", "דני", "רוני", "שרה", "יעל", "אילן", "מירי", "איריס", "ירון", "נועה", "אבי", "איתן", "אורן", "עמית", "אילה", "רות", "משה", "חיים", "ליאת", "שירה"];
    const lastNames = ["כהן", "לוי", "ביטון", "מלכה", "אברהם", "דויד", "שמש", "רוזן", "גולן", "שושן", "ישראלי", "אזולאי", "אברג׳יל", "פרץ", "אדרי"];
    
    // יצירת רשימה ראשונית של משתתפים
    initialParticipantsList = [];
    
    // יצירת 20 משתתפים ראשונים לדוגמה
    for (let i = 0; i < 20; i++) {
        const randomFirstName = firstNames[Math.floor(Math.random() * firstNames.length)];
        const randomLastName = lastNames[Math.floor(Math.random() * lastNames.length)];
        
        // יצירת זמן הצטרפות אקראי ב-3 הימים האחרונים
        const now = new Date();
        const joinTime = new Date(now - Math.floor(Math.random() * 3 * 24 * 60 * 60 * 1000)); // זמן אקראי ב-3 ימים האחרונים
        ...
        const participant = {
            ...
            joinTime: joinTime,
            joinTimeFormatted: joinTime.toLocaleString('he-IL')
        };
        
        // הוספה לרשימה
        initialParticipantsList.push(participant);
    }
    
    // מיון הרשימה לפי זמן הצטרפות (החדש ביותר תחילה)
    initialParticipantsList.sort((a, b) => b.joinTime - a.joinTime);
    
    // עדכון הרשימה במודאל אם הוא קיים
    updateParticipantsListInModal();
    
    console.log(`נוצרה רשימת ${initialParticipantsList.length} משתתפים ראשונית`);
}

// פונקציה לעדכון רשימת המשתתפים במודאל
function updateParticipantsListInModal() {
    // בדיקה אם המודאל קיים
    const participantsList = document.getElementById('participants-list');
    if (!participantsList) {
        return; // אם האלמנט לא קיים, יציאה מהפונקציה
    }
    
    // ניקוי הרשימה
    participantsList.innerHTML = '';
    
    // הוספת כותרת לטבלה אם אין
    if (!document.querySelector('#participants-list .participant-header')) {
        const header = document.createElement('div');
        header.className = 'participant-header';
        header.innerHTML = `
            <div class="participant-name">שם מלא</div>
            <div class="participant-time">זמן הצטרפות</div>
        `;
        participantsList.appendChild(header);
    }
    
    // הוספת כל המשתתפים לרשימה
    initialParticipantsList.forEach(participant => {
        const participantElement = document.createElement('div');
        participantElement.className = 'participant-item';
        participantElement.innerHTML = `
            <div class="participant-name">${participant.firstName} ${participant.lastName}</div>
            <div class="participant-time">${participant.joinTimeFormatted}</div>
        `;
...
function setupUserDetailsForm() {
    ...
    submitButton.addEventListener('click', function(e) {
        ...
        // שמירת פרטי המשתמש
        const saveSuccess = saveUserDetails();
        ...
        // אם השמירה נכשלה, לא עושים כלום - ההודעה מוצגת בפונקציית saveUserDetails
    });
    ...
    function saveUserDetails() {
        try {
            ...
            const userDetails = {
                ...
                deliveryMethod: deliveryMethodElement.value
            };
            ...
            return true; // מחזירים true כדי לסמן שהפעולה הצליחה
        } catch (error) {
            // טיפול בשגיאות אחרות שעלולות להתרחש
...
function updateParticipantsCounter() {
    // עדכון מספר הנרשמים בכל הרכיבים בדף
    const participantsElements = document.querySelectorAll('.participants-count');
    participantsElements.forEach(element => {
        element.textContent = totalParticipants;
    });
