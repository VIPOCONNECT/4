...
function updateSummary() {
    ...
    document.getElementById('summaryGroupPrice').textContent = `₪${finalPrice.toLocaleString()}`;
    ...
    document.getElementById('summaryRemainingPayment').textContent = `₪${Math.max(0, remainingPayment).toLocaleString()}`;
...
function submitUserDetails() {
    ...
    // שליחת הודעת ווצאפ עם פרטי הנרשם
...
function sendWhatsAppNotification(userDetails) {
    try {
        // מספר הטלפון שיקבל את ההודעה - שימו לב שהסרנו את הסימן + מהתחילת המספר
        ...
        const whatsappLink = `https://api.whatsapp.com/send?phone=${adminPhone}&text=${message}`;
        
        // פתיחת חלון חדש עם הקישור לשליחת הודעת ווצאפ
        // השיטה הישירה והאמינה ביותר היא פתיחת חלון חדש
        window.open(whatsappLink, '_blank');
        
        // שמירת הקישור במשתנה גלובלי לצורך בדיקה
        window.lastWhatsAppLink = whatsappLink;
        
        console.log('נשלחה הודעת ווצאפ עם פרטי הנרשם:', userDetails);
